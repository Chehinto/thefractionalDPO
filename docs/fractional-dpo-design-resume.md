...
- **Growth mechanics — three, compounding:**
  1. **Bottom-up (primary, self-serve).** Any accidental DPO who's used the tool can spin up a new tenant for another company (a friend's startup, a side gig) and list themselves as Active DPO there. This organically manufactures fractional DPOs out of the existing user base — sidesteps the classic marketplace cold-start problem (need supply and demand simultaneously) flagged in an earlier session, without requiring you to recruit DPOs yourself.
  2. **Top-down (primary go-to-market motion).** Sell directly to already-established fractional DPOs with an existing client portfolio. Landing one DPO brings their whole book onto the platform at once — N tenants per sale, not one. The £79/£99 pass-through structure (a DPO bills the £79 through their own invoice, framed as continuity insurance: "when I leave, you keep it at £99") is what makes an established DPO comfortable bringing their whole portfolio rather than trialling on one client.
  3. **Vendor-recommendation (compounding, via the DPIA feature).** When a DPIA questionnaire is sent to a vendor, the request can include a note recommending the vendor try AxioVendo to answer it (and future ones) faster — a warm lead for AxioVendo, since a vendor triggering one DPIA will keep being asked for more. Consent design, settled after extended discussion: this is never vendor-targeted marketing consent — it's the DPO's own client configuring what goes in their own outbound message (ordinary B2B service configuration, not data-subject consent under GDPR). Two separate toggles, never bundled: (1) mandatory — authorizes the platform to send the DPIA on the DPO's behalf, branded as such; (2) the AxioVendo-recommendation note, **on by default as a one-time workspace setting** (not asked per-send — asking the same low-stakes question on every DPIA guarantees it gets ignored), disclosed plainly at onboarding, one click to turn off in Settings, never bundled with (1). A bundled or per-send-nagged version was considered and explicitly rejected — it would have converted the DPO's genuine, individualized professional recommendation (the thing that makes the underlying legitimate-interest case survivable) into a systematic default insertion, undermining the same legal basis it depends on.
  - Bottom-up and vendor-recommendation both compound the top-down motion over time: every company a top-down DPO onboards is itself a future source of a bottom-up signup once someone there gets comfortable with the tool, and every vendor recommended to AxioVendo is a future candidate to need their own fractional DPO.
...
1. **Active DPO** (a list per tenant, not a single owner) — full trust, full visibility: register, DPIA, incidents, risk analysis. Can be accidental or fractional; a tenant can have more than one simultaneously (e.g. during a handover).
   ...
   - A fractional DPO's **portfolio dashboard** is a computed rollup across every tenant where they currently appear on this list — not a separate object. **Correction after AxioVendo audit**: "same shape as AxioVendo's admin view" describes the *concept* only, not the mechanism — AxioVendo's admin view works by running with the service role and bypassing RLS entirely, gated on a staff flag. The portfolio rollup must instead be RLS-scoped to tenants the person actually has active membership in. Do not port the service-role/bypass mechanism; a paying fractional DPO must never get de facto cross-tenant superuser access.
...
- **Built**: the vendor-facing DPIA questionnaire round-trip, end to end — hashed/expiring/revocable link tokens, the vendor page, answer capture as returned evidence (not register writes), a full view log including refusals, and an auditor link that exposes only approved register entries. Delivery is not built: the DPO copies the link and sends it themselves.
- **Built**: the tier-2 assignment primitive — one thing pushed to one member, response logged, roster showing who was asked and who has not answered.
...
5. ~~Tier-2 (staff) persistent identity mechanism~~ — **settled and built.** A lightweight account, not a magic link: a `people` row plus a `staff` membership, so history accumulates per person. The tier-3 work settled it by contrast — the *stranger* is the case that should be tokenised, precisely because a token holder must NOT accumulate identity.
6. Prompt B full text — not written.
7. Vendor-facing DPIA questionnaire prompt — ~~link flow~~ **built**; the prompt that generates the questions is still not designed.
...
11. ~~Membership model~~ — **built.** `memberships(tenant_id, person_id, tier, active_from, active_to)` is live, with a GiST exclusion constraint (not a unique index) enforcing at most one live membership per person per tenant, `on delete restrict` to protect provenance, and RLS predicated on membership rather than a single-org lookup. Tenant creation (`create_tenant`) and identity reconciliation (unclaimed roster rows linking to real signups) are also built and tested. Tier 3 (external scoped access) is now built, and token-based as expected rather than a `memberships` row: a hashed token, a short expiry with a database-enforced ceiling, revocation checked live on every request, and no `anon` grant on any table.
12. **Provenance vs. billing as separate objects**: §5 already establishes provenance (companies-advised credential) survives independently of live tenant data. The AxioVendo audit notes this means provenance and the billing record are different objects with different lifetimes — worth confirming that's intended before either gets a schema, since it intersects with the unresolved two-Active-DPO billing question (item 3).
13. **AxioVendo's base schema is not in version control** (only reconstructable from the live database) — needs `scripts/dump_schema.py` (or equivalent) run and committed as `001_base.sql` before treating anything as a clean port. Mechanical, but blocking. Does not block the new product's own schema, which is being built fresh rather than migrated wholesale.
14. **Email confirmation before `create_tenant` is reachable** — tension between AxioVendo's "sign up and start today" positioning and this product's register-credibility argument for requiring confirmation first. Leaning toward requiring it; not yet decided.

---

## 9. Data schema decisions

**Data categories — two separate enums, not one list with a flag.**

Ordinary:
```
contact_details, employment_data, financial_data, technical_usage_data,
customer_commercial_data, education_qualification_data,
communications_data, location_data, image_video_data
```

Special category (Art. 9, closed/statutory list — not to be edited, only used):
```
health_data, biometric_data, genetic_data, racial_ethnic_origin,
religious_philosophical_beliefs, political_opinions,
trade_union_membership, sex_life_or_orientation
```

Plus, treated with the same weight in practice though technically Art. 10:
```
criminal_offence_data
```

Rationale for splitting into two enums rather than one flagged list: the DPIA risk flag becomes a trivial check ("any special category present → flag") rather than a per-value lookup. Implemented as two Postgres enums, same pattern as `dpo_legal_basis` and the membership `tier` enum — not a lookup table. If a real client activity doesn't fit later, that's a migration, not a redesign.

**Confidence-tag semantics** — one definition, shared by Prompt A (batch intake), Prompt B (reconciliation), and the day-to-day elicitation prompt:
- `stated` — a person directly said this, or a document explicitly states it in terms that require no interpretation.
- `inferred` — the model concluded this from context (e.g., "Retool" defaulted to processor-not-controller because it's a well-known SaaS tool); must be shown back to whoever's reviewing and cannot silently become `stated`.
- `unknown` — not established by any source seen so far. Never filled with a plausible-sounding guess. Flagged for the DPO, not defaulted.

Confirmed in practice on the tenancy layer already built: `legal_basis` is `stated` (a person answered it at signup), a tenant's people count is `inferred` (derived from membership rows, not a stated headcount), and an unrecorded sector renders as "Not recorded" rather than being silently dropped — the pattern holds up outside the prompts it was originally written for.

**Canonical processing-activity schema** — the seven fields, now formalized:

```
processing_activity:
  purpose            text, required
  recipient_vendor   text, nullable (first-party activities have none)
  role               enum: controller | processor
  data_categories    ordinary_category[], special_category[]
  data_subjects      enum[]: customers | employees | job_applicants |
                      website_visitors | other
  retention          text (period + basis), nullable
  dpia_risk_flag     boolean, computed from data_categories

  Every field above — except dpia_risk_flag (derived, not tagged) and
  role (always asserted, never guessed) — carries {value, confidence,
  evidence} per the confidence-tag definitions above.
```

---

## 10. Roadmap beyond GDPR

**Sequencing decided: GDPR now, ISO/IEC 27701 committed as the second phase. ISO/IEC 27001 explicitly parked, not committed** — treated as a possible later bet only if 27701 proves the model, not an equally-weighted third step. Reasoning: 27701 stays in a fractional DPO's actual expertise and in this product's differentiated territory; 27001 (full information-security management) is Vanta's most mature, most heavily-integrated product line — the wrong ground to fight on with a much smaller team.

**Why 27701 now, not later:** ISO/IEC 27701 was republished October 2025 as a genuinely standalone standard — no longer an extension requiring an ISO 27001 ISMS as prerequisite, own complete requirements, own Annex A controls. This removes the exact barrier that made it a non-starter for a small company with no security program before. Directly relevant to the self-serve/accidental-DPO segment (§4/§6): they can now pursue real PIMS certification without first needing an ISMS they have no reason to build. Framed as a more defensible moat than continuing to add GDPR-adjacent features, since it's newly accessible and still underserved.

**Architectural note, not to be assumed free:** a controls/management-system standard is a structurally different object from the register, not an extension of it. `processing_activity` records *what processing happens*; a 27701 implementation needs a parallel `control` table (framework, control ID, status, evidence, owner, continual-improvement tracking) — referencing shared evidence where it genuinely overlaps (retention, data categories) but not the same schema.

**Reuse candidate, better than the ROPA port was:** AxioVendo's coverage-checklist engine (13 categories, `typicalQuestions`, owner hints, amber/green scored against expected fields per category) is already a controls-coverage-against-a-checklist tracker in substance — just aimed at vendor-questionnaire evidence. Retargeting its rows to 27701 Annex A controls is a much cleaner port than the extraction cascade's adaptation to ROPA was; most of the mechanism should transfer close to as-is.

**Open, not yet answered:** whether a fractional DPO is typically also the person implementing a client's PIMS, or whether this pulls in a different professional (a fractional CISO/GRC consultant) wearing the same tenant architecture — worth answering before this phase is designed in detail, not assumed.

This is the schema Prompt A, Prompt B, and the day-to-day elicitation prompt all write into, and what the register's underlying table is built from.
